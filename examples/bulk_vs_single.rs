...
fn main() {
    ...
    let rsi = bulk_rsi(&data, SmoothedMovingAverage, 14).expect("Failed to calculate bulk RSI");
    println!("Bulk RSIs: {:?}", rsi);

    // new price comes in
    data.push(5769.21);
    let single_rsi = single_rsi(&data[40..], SmoothedMovingAverage).expect("Failed to calculate single RSI");
