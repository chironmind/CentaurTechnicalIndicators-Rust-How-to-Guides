...
fn main() {
    ...
    let bands = bulk_md_bands(&prices, StandardDeviation, 2.0, 0.0, 5).expect("Failed to calculate bands");
    ...
    let next_band = single_md_bands(&prices[247..], StandardDeviation, 2.0, bands.last().unwrap().1).expect("Failed to calculate bands");
